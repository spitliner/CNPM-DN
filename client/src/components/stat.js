...
function groupData(orderList) {
    ...
    orderList.forEach(element => {
        let time = element.date.split('T')[0].split('-');

        let day = '0' == time[2][0]?time[2].slice(1):time[2];
        let month = '0' == time[1][0]?time[1].slice(1):time[1];
        let year = time[0];
    ...
    render () {
        return (
            <ResponsiveContainer width="100%" aspect={4.5}>
                <LineChart data={this.props.renderData}
...
class RenderChart extends React.Component {
    constructor(props) {
        super(props);
        let Lists = [], yearList = [], selectYear = "", selectMonth = "", monthList = [], renderData = []
        if (this.props.data && this.props.data.length >= 1) {
            ...
            renderData = setData(selectYear, selectMonth, Lists.orderMap, this.props.dataKey)
        }
    ...
    render () {        
        return (
            <div className="displayChart">
                <h2>{this.props.statement} {this.state.monthName[this.state.selectMonth]} {this.state.selectYear}</h2>
                <div>
                    <div className="option">
                        <Select options={this.state.monthList} onChange={event => {
                            this.setState({
                                selectMonth: event.value,
                                renderData : setData(this.state.selectYear, event.value, this.state.Lists.orderMap, this.props.dataKey)
                            })
                        } } />
                    </div>
                    <div className="option">
                        <Select options={this.state.yearList} onChange={event => {
                            this.setState({
                                selectYear : event.value,
                                monthList : this.writeToSelect(this.state.Lists.trackList.get(event.value))
                            })
                        } } />
                    </div>
                </div>
                <div>
                    <LineRechartComponent renderData ={this.state.renderData} dataKey={this.props.dataKey}/>
                </div>
            </div>
        )
    }
...
class RenderStat extends React.Component {
    ...
    render() {
        return (
        <div className="stat">
                <RenderChart data={this.state.orders} statement="Orders statistics per day in" dataKey="Orders amount"/>
                <RenderChart data={this.state.reservations} statement="Reservations statistics per day in" dataKey="Reservations"/>
        </div>
        );
    }
} 
